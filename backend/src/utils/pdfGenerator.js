...
class PDFGenerator {
  ...
  static _addHeader(doc, scan) {
    doc
      .fontSize(24)
      .fillColor('#1a1a1a')
      .text('NoVuln - Security Code Review Report', { align: 'center' })
      .moveDown(0.2);

    doc
      .fontSize(13)
      .fillColor('#4f46e5')
      .text('Enterprise AI SAST Platform', { align: 'center' })
      .moveDown(0.5);

    doc
      .fontSize(11)
      .fillColor('#666666')
      .text(`Language: ${scan.language ? scan.language.toUpperCase() : 'MULTI-FILE'}`, { align: 'center' })
      .text(`Generated: ${new Date(scan.createdAt || scan.timestamp || Date.now()).toLocaleString()}`, { align: 'center' })
      .text(`Scan ID: ${scan._id || 'NOVULN-SCAN'}`, { align: 'center' })
      .moveDown(1.5);
  ...
  static _addFooter(doc) {
    ...
    for (let i = 0; i < pages.count; i++) {
      ...
      doc
        .text(
          'Generated by NoVuln - Enterprise AI SAST Platform',
          50,
          doc.page.height - 35,
          { align: 'center' }
        );
